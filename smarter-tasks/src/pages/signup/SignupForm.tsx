  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('API Error:', errorData); // Log error response
        throw new Error('Sign-up failed');
      }

      const data = await response.json();
      console.log('Response Data:', data); // Log entire response data to inspect its structure

      // Check if token and user data exist before saving to localStorage
      if (data.token) {
